...
def _compute_special_connection_badges(history):
    """
    {room_id: {"label", "bg", "fg", "partner_id"}} for every room
    whose Location/Detail has a special-connection trait (Lift/Secret
    Passage/Fireplace) - computed fresh on every render rather than
    stored anywhere, since it's entirely derivable from
    special_link_id / detail.

    Shows a badge even before anything is actually connected yet -
    same letter, same shape, just a neutral/muted color and no
    partner to click through to (`partner_id: None`) - so the map
    consistently flags "this room has one of these" everywhere, not
    only once it's been used for the first time.

    Once actually linked, Lift and Secret Passage (fixed pairs) each
    get their own sequential label per kind ("L1", "L2", ... / "S1",
    "S2", ...), numbered in order of the lower room id in the pair -
    stable across renders without needing to store the number
    anywhere. Colors, though, are assigned from a single sequence
    shared across *both* kinds, in the order each pair was actually
    established - not restarted per kind - so e.g. the first Lift
    pair and the first Secret Passage pair (often created around a
    similar time) don't both default to the same first color just
    because each is "first" within its own kind. Fireplace never
    gets a number: every room with that detail belongs to the one
    single network the rules describe, so there's never more than
    one to tell apart.
    """
    badges = {}
    ...
    letters = {"lift": "L", "secret_passage": "S"}
    all_pairs = sorted(
    ...
    for color_index, (kind, (a, b)) in enumerate(all_pairs):
        ...
        label = f"{letters[kind]}{kind_counts[kind]}"
        badges[a] = {"label": label, "bg": bg, "fg": fg, "partner_id": b}
        badges[b] = {"label": label, "bg": bg, "fg": fg, "partner_id": a}

    unc_bg, unc_fg = _UNCONNECTED_BADGE_COLOR
    for kind, room_ids in unconnected_by_kind.items():
        for room_id in room_ids:
            badges[room_id] = {
                "label": letters[kind], "bg": unc_bg, "fg": unc_fg, "partner_id": None,
            }
    ...
    if len(fireplace_rooms) >= 2:
        ...
        for room in fireplace_rooms:
            ...
            badges[room["id"]] = {"label": "F", "bg": bg, "fg": fg, "partner_id": other["id"]}
    elif len(fireplace_rooms) == 1:
        badges[fireplace_rooms[0]["id"]] = {
            "label": "F", "bg": unc_bg, "fg": unc_fg, "partner_id": None,
        }
    ...
    for entry in flat:
        ...
        if badge:
            ...
            badges_html.append(
                ...
                f'style="left:{badge_cx - 10:.1f}px; top:{badge_cy - 10:.1f}px; '
                f'background:{badge["bg"]}; color:{badge["fg"]};"{badge_attrs} '
                f'title="{badge_title}">{badge["label"]}</span>'
            )
