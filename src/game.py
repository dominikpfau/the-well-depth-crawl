...
def _room_has_treasure(room) -> bool:
    ...
    item across any of its treasure entries (see "treasures") - used
    for the Dungeon Map's real treasure marker. "open"/"monster"
    entries are always visible; a "ransack" entry only counts once
    the room's been searched (see "ransacked" / "ransack_room") -
    for a location/detail with DETAIL_TRAITS'/LOCATION_TRAITS'
    "guaranteed_treasure" (lying out in the open, e.g. "Treasure
    Pile"/"Portcullis") that's immediately, from the moment it's
    generated (see _generate_room). Individually removing items (see
    "remove_treasure_item") can empty an entry's item_list out
    entirely without clearing found_treasure itself (the quality/roll
    info is still meaningful history), so this checks item_list
    specifically rather than just found_treasure's truthiness.
    """
    for entry in room.get("treasures", []):
        if entry["context"] == "ransack" and not room.get("ransacked"):
            continue
...
def _room_treasure_pending_search(room) -> bool:
    """
    True if this room has a "ransack"-context treasure entry (its own
    hidden treasure, on record since generation - see _generate_room)
    that hasn't been revealed yet - regardless of whether anything
    was actually found there, since that's exactly the point: showing
    a marker only when treasure is actually there (even a distinct
    "nothing here" non-marker) would let the Dungeon Map spoil the
    search before it happens. Used for the Dungeon Map's grey "?"
    marker, shown instead of (never alongside) the real treasure
    marker. Doesn't consider "open"/"monster" entries at all - they're
    never gated behind a search in the first place, so there's
    nothing "pending" about them.
    """
    if room.get("ransacked"):
        return False
    return any(entry["context"] == "ransack" for entry in room.get("treasures", []))
...
def _generate_room(
    ...
    room = {
        ...
        # with a "context" ("ransack" / "open" / "monster") so the
        # room card can group and label them separately rather than
        # dumping every item into one undifferentiated list (see
        # _render_room_treasures). Every item across every entry in
        # this same list shares one running id sequence (assigned via
        # _next_treasure_item_id below) - not restarted per entry -
        # since "remove_treasure_item" only takes a room id and an
        # item id, no separate "which entry" dimension, so ids need
        # to stay unique across the whole room, not just within the
        # one generate_treasure() call that produced them.
        "treasures": [],
        # Whether this room's *hidden* treasure (the "ransack"-context
        # entry, if there is one) has actually been revealed to the
        # players yet - "open" and "monster" entries are never gated
        # by this, they're visible the moment they exist (see
        # _render_room_treasures). Starts False - the room card shows
        # a "Ransack Room" button in the "ransack" entry's place until
        # "ransack_room" flips this - except when there's definitely
        # nothing to search for in the first place: a location/detail
        # with the "no_treasure" modifier (already known to be empty,
        # e.g. "Looted") sets this True immediately further down
        # instead. A "guaranteed_treasure" location/detail (lying out
        # in the open, e.g. "Treasure Pile"/"Portcullis") does *not*
        # skip this on its own - its "open" entry is always visible
        # regardless, but the room can still separately have its own
        # hidden stash worth searching for, same as any other room.
        "ransacked": False,
    ...
    def _next_treasure_item_id():
        # Room-scoped counter, closed over the room dict itself
        # (already built above) rather than passed around separately
        # - see the "treasures" field comment for why ids must stay
        # unique across every entry in the room, not just within one.
        used = [
            item["id"]
            for entry in room["treasures"]
    ...
    def _renumber_items(found_treasure):
        if not found_treasure:
            return
        next_id = _next_treasure_item_id()
...
def handle_action(
    ...
    if action == "room":
        ...
        if target_id is not None:
            target_room = _room_by_id(crawl_history, target_id)
            if target_room is not None and target_room.get("treasures"):
...
def _render_room_treasures(room, show_rolls, room_id, interactive):
    ...
    has to search for *and* loot already sitting on a monster they've
    beaten. Only the "ransack" entry (the room's own hidden treasure)
    is ever gated behind a "Ransack Room" button, and only when
    `interactive` (Crawling Mode) and the room hasn't been searched
    yet - "open" (lying out in plain view) and "monster" (dropped by
    something already defeated) are both visible the moment they
    exist, same as the Encounter section itself already is. Returns
    "" if the room has no treasure entries at all (roll_treasure was
    off), same as before this grouped into contexts.
    """
    blocks = []
    for entry in room.get("treasures", []):
        label = _TREASURE_CONTEXT_LABELS.get(entry["context"], entry["context"].title())

        if entry["context"] == "ransack" and interactive and not room.get("ransacked"):
            # What's under this entry was already rolled the moment
            # this room was generated (same as ever - see
            # _generate_room), it's just not shown until
            # "ransack_room" reveals it, one way or the other
            # (including "nothing here"). Only reachable here at all
            # when interactive; the Location Generator's own room has
            # no persistent state to search later, so its hidden
            # treasure (when rolled for at all) is shown immediately
            # instead, same as "open"/"monster" always are.
            body = (
